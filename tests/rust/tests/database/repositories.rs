...
async fn test_space_repository_create_and_get() {
    ...
    SpaceRepository::create(&repo, &space).await.expect("Failed to create space");
    
    // Read
    let loaded = SpaceRepository::get(&repo, &space_id).await.expect("Failed to get space");
...
async fn test_space_repository_update() {
    ...
    // Update
    let mut updated_space = SpaceRepository::get(&repo, &space_id).await.unwrap().unwrap();
    updated_space.name = "Updated Name".to_string();
    SpaceRepository::update(&repo, &updated_space).await.expect("Failed to update space");
    
    // Verify
    let reloaded = SpaceRepository::get(&repo, &space_id).await.expect("Failed to reload space");
...
async fn test_space_repository_delete() {
    ...
    // Delete
    SpaceRepository::delete(&repo, &space_id).await.expect("Failed to delete space");
    
    // Verify deleted
    let deleted = SpaceRepository::get(&repo, &space_id).await.expect("Failed to check deleted");
...
async fn test_space_repository_list_all() {
    ...
    let initial_spaces = SpaceRepository::list(&repo).await.expect("Failed to list initial spaces");
    ...
    let all_spaces = SpaceRepository::list(&repo).await.expect("Failed to list spaces");
...
async fn test_space_repository_default_space() {
    ...
    SpaceRepository::create(&repo, &my_default_space).await.unwrap();
    SpaceRepository::set_default(&repo, &my_default_id).await.unwrap();
    
    // Get default - should be our space
    let found_default = SpaceRepository::get_default(&repo).await.expect("Failed to get default");
...
async fn test_space_repository_set_default() {
    ...
    SpaceRepository::set_default(&repo, &space1_id).await.expect("Failed to set default");
    
    let default = SpaceRepository::get_default(&repo).await.unwrap().unwrap();
    assert_eq!(default.id, space1_id);
    
    // Change default to space2
    SpaceRepository::set_default(&repo, &space2_id).await.expect("Failed to change default");
